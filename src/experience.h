...
using ExpKey   = Judas::Key;
using ExpMove  = Judas::Move;
using ExpValue = Judas::Value;
using ExpDepth = Judas::Depth;
...
struct ExpEntryEx: Current::ExpEntry {
    ...
    std::pair<int, bool> quality(Judas::Position& pos, int evalImportance) const;
};
...
void show_exp(Judas::Position& pos, bool extended);
