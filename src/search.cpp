...
double exploration_factor = 0.2; // Valore predefinito
bool dynamic_exploration = true; // Modalità dinamica attiva di default
LimitsType Limits;
}
...
// Judas at various skill levels and various versions of the Stash engine.
...
void update_exploration_factor(const Position& pos, int depth, int timeLeftMs) {
    if (!dynamic_exploration) return; // Esce se la modalità dinamica è disabilitata

    // Inizia con il valore configurato dall'utente
    double base_factor = Options["Exploration Factor"];
    exploration_factor = base_factor;

    // Adatta in base al tempo rimanente
    if (timeLeftMs > 60000) // Tempo abbondante
        exploration_factor += 0.1;
    else if (timeLeftMs < 10000) // Poco tempo
        exploration_factor -= 0.1;

    // Adatta in base alla profondità
    exploration_factor -= 0.01 * depth;

    // Adatta in base alla complessità della posizione (ad esempio, densità di attaccanti)
	Square kingSq = pos.square<KING>(pos.side_to_move());
    int attackers = popcount(pos.attackers_to(kingSq));
    if (attackers > 2) // Posizioni tattiche
        exploration_factor += 0.1;

    // Clamp tra 0.0 e 1.0
    exploration_factor = std::clamp(exploration_factor, 0.0, 1.0);
}
           ...
           && !(Limits.depth && mainThread && rootDepth > Limits.depth))
    {
        ...
        for (pvIdx = 0; pvIdx < multiPV && !Threads.stop; ++pvIdx)
        {
            ...
            while (true)
            {
                ...
                bestValue = Judas::search<Root>(rootPos, ss, alpha, beta, adjustedDepth, false);
