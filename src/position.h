...
class Position {
   public:
    ...
    // Funzioni per caratteristiche della posizione
    int mobility_score() const;                 // Calcola la mobilità
    int pawn_structure_score() const;          // Valuta la struttura pedonale
    bool is_symmetric() const;                 // Controlla la simmetria
    int king_safety_score(Color sideToMove) const; // Calcola la sicurezza del re

    // Funzioni per caratteristiche avanzate della posizione
    bool is_isolated(Square s, Bitboard pawns) const;
    bool is_doubled(Square s, Bitboard pawns) const;
    Piece mirrored_piece(Square s) const;
    Square king_square(Color c) const;
    bool is_open_file(Square s) const;
    Bitboard attacking_pieces(Color c, Square s) const;

    // Funzioni per identificazione sacrifici
    bool is_sacrifice(Move m) const;           // Verifica se una mossa è un sacrificio
    bool leads_to_attack() const; // Verifica se esiste almeno una mossa che porta ad un attacco
    bool is_sacrifice() const;  // Verifica se ci sono sacrifici promettenti nella posizione
