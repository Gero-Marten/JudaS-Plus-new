...
bool Position::is_sacrifice(Move m) const {
    // Ottieni i pezzi coinvolti nella mossa
    Piece moved = moved_piece(m);
    Piece captured = captured_piece();

    // Verifica che il pezzo mosso sia valido
    if (moved == NO_PIECE) {
        return false;
    }

    // Calcola il valore del pezzo catturato (0 se nessun pezzo catturato)
    int capturedValue = (captured == NO_PIECE) ? 0 : PieceValue[captured];

    // Controlla se la mossa è un sacrificio
    return PieceValue[moved] > capturedValue;
}

bool Position::leads_to_attack() const {
    // Ottieni tutte le mosse del lato in movimento
    Bitboard allPieces = pieces(side_to_move());

    while (allPieces) {
        Square from = pop_lsb(allPieces); // Ottieni la posizione di un pezzo
        Bitboard possibleMoves = attackers_to(from, pieces()); // Genera mosse pseudo-legali

        while (possibleMoves) {
            Square to = pop_lsb(possibleMoves); // Ottieni la casella di destinazione
            int attackersCount = popcount(attackers_to(to) & pieces(side_to_move()));

            // Considera promettente se ci sono abbastanza attaccanti
            if (attackersCount > 2) {
                return true; // Mossa promettente trovata
            }
        }
    }

    return false; // Nessuna mossa promettente trovata
}

bool Position::is_sacrifice() const {
    // Ottieni tutti i pezzi del lato in movimento
    Bitboard allPieces = pieces(side_to_move());
    ...
    while (allPieces) {
        ...
        while (possibleMoves) {
            pop_lsb(possibleMoves); // Ottieni la prossima casella (ma non salvare il valore)

            // Verifica se la mossa porta ad un attacco promettente
            if (leads_to_attack()) {
                return true; // Sacrificio promettente trovato
            }
        }

        // Controlla sacrifici basati su pezzi nemici
        Bitboard enemyPieces = pieces(~side_to_move());
        if (popcount(attackers_to(from) & enemyPieces) > 0 && PieceValue[piece] >= 300) {
            return true; // Sacrificio promettente trovato
        }
    }

    return false; // Nessun sacrificio rilevato
}
